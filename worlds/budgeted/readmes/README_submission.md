...
The task-keyed source of truth is `../analysis/<task>/band_supports.json` (task metadata + a list of
evals, each with its own stats), so the numbers are reproducible and never hand-recomputed. Records +
this table are GENERATED by `band_resolution.py --emit`; do not hand-edit rows, re-emit.
